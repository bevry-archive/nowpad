(function(window,undefined){
	...
	nowpadClient.prototype = {
		...
		request: function(){
			...
			// Grab a Lock
			timeoutInterval = setTimeout(timeoutCallback,me.timeoutDelay);
			window.now.lock(function(_result){
				...
				if ( _result ) {
					...
					me.selectionRange = me.getSelectionRange();
		...
		sync: function(){
			...
			if ( this.getValue() !== newCurrentValue ) {
				...
				newSelectionRange = this.getSelectionRange();
				...
				console.log(
					...
					[this.selectionRange.selectionStart,this.selectionRange.selectionEnd]
				);
		...
		apply: function(_state,_value){
			...
			if ( _value !== patchValue && _state.client !== this.id ) {
				...
				console.log('ignored cursor:', [_value !== patchValue], [_state.client !== this.id]);
			}
